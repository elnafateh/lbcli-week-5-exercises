# What is the redeem script in this transaction
 transaction=""
# Decode raw tx and grab the scriptSig asm
asm=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.asm')

# The redeem script is the last item/push in the scriptSig asm
redeem_script=$(echo "$asm" | awk '{print $NF}')

echo "$redeem_script"
